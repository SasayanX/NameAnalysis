...
1. **【最優先】名乗りと漢字の霊意:**
   ...
   - その直後、${kanjiSourceText}あなたの直感で選んだ漢字一文字の霊意（言霊）を、**詩的な比喩**で必ず言及してください。これにより最高のパーソナライゼーションを実現します。
...
2. **鑑定文の統合と構成:**
   ...
   - **【論理的根拠の統合】** 吉格の強みと、その裏にある課題を同時に指摘し、現実的な深みを与えてください。
   - **【課題と解決策】** 弱い要素（五行のweakElementや凶格の傾向）を指摘し、それを補うための「祈りの味わい」と「日々の具体的な行動」を提示してください。

3. **JSONフィールドの対応:**
   - **fortune (約400字):** 上記要件1,2,4,5をすべて満たした総合鑑定文。**内容が変わる文節の間には空行（改行2つ）を入れてください。**
   - **personality (約100字):** 穏やかで温かい口調で、感情の深みや繊細さに触れる洞察を含めてください。
   - **talents (約100字):** 金龍の流れに乗り、独立や集団指導に活かせる才能をポジティブに描写してください。
   - **challenges (約100字):** weakElementや凶格の傾向を具体的課題として提示し、「焦らず、地に足をつける」実践的な解決策を示してください。
   - **luckyElement:** 五行分析の結果を踏まえ、運勢を動かすラッキーな要素を簡潔に示してください。
   - **advice (約50字):** 日々の意識と行動で運勢を変えられるというメッセージと、焦らず進む祈りを込めた実践的アドバイスを生成してください。

4. **【補強要素B：五行に基づいた具体的アクションの提案】**
   - ${weakElement ? `弱い要素は「${weakElement}」です。` : '弱い要素の情報が不足している場合でも、直感的に不足しがちな要素を推定してください。'}
   - **fortuneの最後の段落の前に**、weakElementを補うために今日からできる具体的で簡単なアクションを3つ提示してください（例：「[アクション1], [アクション2], [アクション3]」）。

5. **締めの祈り文:** 鑑定文の最後は、「今日という一日が、あなた様にとって、内なる調和を見出し、揺るぎない自信へと繋がる、豊かな時間となりますように。あなたの言葉には、未来を動かし、運命を拓く尊い力が宿っています。」のような、希望を持てる祈り文で締めてください（実際の文言は運勢に合わせて調整可）。
