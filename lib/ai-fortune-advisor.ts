...
function generatePreDaiKichiAdvice(categories: any[], gender: "male" | "female"): string {
  ...
  const preDaiKichiPatterns = [
    {
      ...
      advice: "🌟 外格・総格の前プ大吉は「最強の運気」の象徴です！この組み合わせは、人間関係と人生全体の運気が最高レベルに達していることを示します。特に女性にとっては、良縁と社会的成功の両方を手に入れることができる特別な運気です。"
    },
    {
      condition: (cats: any[]) => cats.some(c => c.name === "天格" && c.fortune === "大吉") && cats.some(c => c.name === "人格" && c.fortune === "大吉"),
      advice: "🌟 天格・人格の前プ大吉は「リーダーシップ」の象徴です！この組み合わせは、生まれ持った才能と人格の両方が最高レベルに達していることを示します。特に男性にとっては、事業や組織で大きな成功を収めることができる特別な運気です。"
    },
    {
      condition: (cats: any[]) => cats.some(c => c.name === "地格" && c.fortune === "大吉") && cats.some(c => c.name === "総格" && c.fortune === "大吉"),
      advice: "🌟 地格・総格の前プ大吉は「家庭運」の象徴です！この組み合わせは、家庭生活と人生全体の運気が最高レベルに達していることを示します。特に女性にとっては、家庭と仕事の両方で幸せを掴むことができる特別な運気です。"
    },
    {
      condition: (cats: any[]) => cats.some(c => c.name === "天格" && c.fortune === "大吉") && cats.some(c => c.name === "総格" && c.fortune === "大吉"),
      advice: "🌟 天格・総格の前プ大吉は「完璧な運気」の象徴です！この組み合わせは、生まれ持った運気と人生全体の運気が最高レベルに達していることを示します。あらゆる面で成功と幸せを手に入れることができる特別な運気です。"
    },
    {
      condition: (cats: any[]) => cats.some(c => c.name === "人格" && c.fortune === "大吉") && cats.some(c => c.name === "地格" && c.fortune === "大吉"),
      advice: "🌟 人格・地格の前プ大吉は「バランス」の象徴です！この組み合わせは、人格と家庭生活の運気が最高レベルに達していることを示します。特に男性にとっては、仕事と家庭の両方で成功を収めることができる特別な運気です。"
    },
    {
      condition: (cats: any[]) => cats.some(c => c.name === "外格" && c.fortune === "大吉") && cats.some(c => c.name === "地格" && c.fortune === "大吉"),
      advice: "🌟 外格・地格の前プ大吉は「人間関係」の象徴です！この組み合わせは、人間関係と家庭生活の運気が最高レベルに達していることを示します。特に女性にとっては、良縁と家庭の幸せの両方を手に入れることができる特別な運気です。"
    }
  ]
  ...
  // デフォルトの前プ大吉アドバイス
  return `\n\n🌟 前プ大吉の特別な運気について：複数の格で大吉を獲得している${gender === "male" ? "男性" : "女性"}は、非常に稀で特別な運気の持ち主です。この運気を最大限に活かすためには、自信を持って行動し、周囲の人々との調和を大切にすることが重要です。`
}
