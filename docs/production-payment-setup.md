...
3. **サブスクリプションプランの作成とID確認**
   ...
   **重要**: 新しく作成されるプランには**3日間の無料トライアル期間**が含まれます。
   - Phase 0: トライアル期間（3日間、0円）
   - Phase 1: 通常の課金期間（1ヶ月、330円/550円）
   
   **⚠️ 既存のプランがある場合**: 
   - 既存のプランIDを環境変数から削除してください
   - 新しいプランを作成してください（トライアル期間付き）
   - 新しいプランIDを環境変数に設定してください
   
   **注意**: Square APIの`phases`構造で`periods`フィールドが正しくない場合は、エラーメッセージを確認して調整が必要です。
